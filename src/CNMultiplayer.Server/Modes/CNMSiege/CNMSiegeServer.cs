...
namespace CNMultiplayer.Server.Modes.CNMSiege
{
    public class CNMSiegeServer : MissionMultiplayerGameModeBase, IAnalyticsFlagInfo, IMissionBehavior
    {
        ...
        public const int DefenderGoldBonusOnFlagRemoval = 90; //守城方移除旗帜金币补偿
        ...
        private const int AttackerFlagGoldHoldMax = 300; //攻城方持有旗帜金币最大值

        private const int DefenderFlagGoldHoldMax = 300; //守城方持有旗帜金币最大值

        private const float radius = 14f; //旗帜半径
        ...
        private class ObjectiveSystem
        {
            ...
            {
                string text = objectiveEntity.Tags.FirstOrDefault((x) => x.StartsWith("mp_siege_objective_")) ?? "";
                bool flag = false;
                for (int i = 0; i < 2; i++)
                {
        ...
        private void DestructableComponentOnHitTaken(DestructableComponent destructableComponent, Agent attackerAgent, in MissionWeapon weapon, ScriptComponentBehavior attackerScriptComponentBehavior, int inflictedDamage)
        {
            ...
            if (attackerScriptComponentBehavior is BatteringRam batteringRam)
            {
                ...
                {
                    float contribution = inflictedDamage / (float)userCountNotInStruckAction;
                ...
                if (attackerAgent.CurrentlyUsedGameObject != null && attackerAgent.CurrentlyUsedGameObject is StandingPoint standingPoint)
                {
                    ...
                    if (firstScriptOfTypeInFamily != null)
                    {
                        for (int i = 0; i < _lastReloadingAgentPerRangedSiegeMachine.Length; i++)
                        {
                            ...
                            {
                                _objectiveSystem.AddContributionForObjective(root, _lastReloadingAgentPerRangedSiegeMachine[i].Item2.MissionPeer, inflictedDamage * 0.33f);
                            }
                        }
                    }
                }
        ...
        private void DestructableComponentOnDestroyed(DestructableComponent destructableComponent, Agent attackerAgent, in MissionWeapon weapon, ScriptComponentBehavior attackerScriptComponentBehavior, int inflictedDamage)
        {
            ...
            float num = allContributorsForSideAndClear.Sum((ac) => ac.Value);
            ...
            foreach (KeyValuePair<MissionPeer, float> item in allContributorsForSideAndClear)
            {
                ...
                if (goldGainsFromObjectiveAssist > 0)
                {
                    ...
                    list.Add(item.Key);
                    OnObjectiveGoldGained?.Invoke(item.Key, goldGainsFromObjectiveAssist);
                }
            }
            ...
            OnDestructableComponentDestroyed?.Invoke(destructableComponent, attackerScriptComponentBehavior, list.ToArray());
        }
                    ...
                    else if (item.MovementComponent.HasArrivedAtTarget)
                    {
                        ...
                        foreach (KeyValuePair<MissionPeer, float> item3 in allContributorsForSideAndClear)
                        {
                            ...
                            if (goldGainsFromObjectiveAssist > 0)
                            {
                                ...
                                OnObjectiveGoldGained?.Invoke(item3.Key, goldGainsFromObjectiveAssist);
                            }
                        }
                    }
                    else
        ...
        protected override void HandleNewClientAfterSynchronized(NetworkCommunicator networkPeer)
        {
            ...
            foreach (FlagCapturePoint flag in AllCapturePoints.Where((cp) => !cp.IsDeactivated))
            {
        ...
        private void GainGoldForPlayers() //移除旗帜给予玩家金币奖励
        {
            foreach (NetworkCommunicator networkPeer in GameNetwork.NetworkPeers)
            {
                ...
                if (component?.Team?.Side == BattleSideEnum.Defender)
                {
                    ...
                    list.Add(new KeyValuePair<ushort, int>((ushort)GoldGainFlags.ObjectiveCompleted, DefenderGoldBonusOnFlagRemoval)); //防守方旗帜移除金币
                ...
                else if (component?.Team?.Side == BattleSideEnum.Attacker)
                {
                    ...
                    list.Add(new KeyValuePair<ushort, int>((ushort)GoldGainFlags.ObjectiveCompleted, AttackerGoldBonusOnFlagRemoval)); //攻击方旗帜移除金币
                    GameNetwork.BeginModuleEventAsServer(component.Peer);
                    GameNetwork.WriteMessage(new GoldGain(list));
                    GameNetwork.EndModuleEventAsServer();
                    list.Clear();
        ...
        private void GainGoldInFlagRange(Agent agent)
        {
            if (agent.MissionPeer.Representative.Gold < AttackerFlagGoldHoldMax && agent.Team.IsAttacker && _dtFlagGoldTime < ObjectiveCheckPeriod)
            {
                ...
                list.Add(new KeyValuePair<ushort, int>((ushort)GoldGainFlags.ObjectiveCompleted, AttackerFlagGoldGain)); //进攻方争夺旗帜金币奖励
            ...
            else if (agent.MissionPeer.Representative.Gold < DefenderFlagGoldHoldMax && agent.Team.IsDefender && _dtFlagGoldTime < ObjectiveCheckPeriod)
            {
                ...
                list.Add(new KeyValuePair<ushort, int>((ushort)GoldGainFlags.ObjectiveCompleted, DenfenderFlagGoldGain)); //防守方方争夺旗帜金币奖励
                GameNetwork.BeginModuleEventAsServer(agent.MissionPeer.Peer);
                GameNetwork.WriteMessage(new GoldGain(list));
                GameNetwork.EndModuleEventAsServer();
                list.Clear();
