...
{
    [HarmonyPatch(typeof(HeroClassVM), "UpdateEnabled")]//根据战场兵种比例锁定玩家可选兵种。
    ...
    internal class Patch_UpdateEnabled
    {
        public static bool Prefix(HeroClassVM __instance, MissionMultiplayerGameModeBaseClient ____gameMode)
        {
            ...
            if (Id == "Ranged" && Ranged > Sum * archerPer || Id == "Cavalry" && Cavalry > Sum * cavalryPer || Id == "HorseArcher" && HorseArcher > Sum * horseArcherPer)
            {
                flag = false;
