...
namespace CNMultiplayer
{
    public class CNM_MissionMultiplayerSiege : MissionMultiplayerGameModeBase, IAnalyticsFlagInfo, IMissionBehavior
    {
        ...
        private const int DefenderFlagGoldHoldMax = 120; //守城方持有旗帜金币最大值
        ...
        public override void AfterStart()
        {
            ...
            foreach (FlagCapturePoint allCapturePoint in AllCapturePoints)
            {
                if (allCapturePoint.FlagIndex >= 7) //1.1.2.2版本没有锁点设定开局禁用CDEFG旗帜，取一个不可能达到的值7
                {
                ...
                foreach (FlagCapturePoint item in AllCapturePoints.Where((FlagCapturePoint flag) => flag != _masterFlag && !flag.IsDeactivated && GetFlagOwnerTeam(flag).Side == BattleSideEnum.Attacker))
                {
                    ...
                    */ //1.1.2.2版本没有锁点机制
                    ...
                    foreach (NetworkCommunicator networkPeer in GameNetwork.NetworkPeers)
                    {
                        ...
                        {
                            ChangeCurrentGoldForPeer(component, GetCurrentGoldForPeer(component) + 300 / flagnum);//移除旗帜的金币数(防守方)，阶梯金币数与战场上剩余的旗帜数量挂钩
                ...
                if (!flagCapturePoint.IsDeactivated)
                {
                    ...
                    while (proximityMapSearchStruct.LastFoundAgent != null)
                    {
                        ...
                        if (!lastFoundAgent.IsMount && lastFoundAgent.IsActive() && num2 <= radius * radius && !lastFoundAgent.IsAIControlled)
                        {
                            ...
                            if (!flagCapturePoint.IsFullyRaised && ((lastFoundAgent.MissionPeer.Representative.Gold < AttackerFlagGoldHoldMax && lastFoundAgent.Team.IsAttacker) || (lastFoundAgent.MissionPeer.Representative.Gold < DefenderFlagGoldHoldMax && lastFoundAgent.Team.IsDefender)) && (_dtSumCheckMorales % 0.5f < ObjectiveCheckPeriod))//设定占旗获取金币速率
                            {
